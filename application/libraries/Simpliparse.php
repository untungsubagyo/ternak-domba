	...
	function build_select_common($name,$data,$value,$mid,$title,$tamb='',$delimiter=' - ')
	{
		...
		if(is_array($mid))
		{
		   ...
		   {	
		      ...
		      foreach($mid as $val)
    		      {
    		        if($row->$val != '') $str[] = $row->$val;
    		      }
    		      $str_jadi = implode($delimiter,$str);
    		      
    		      if($row->$value === $value) $selected="selected";
    		      else $selected="";
    		      
    		      $opt='<option value=\''.$row->$value.'\' '.$selected.' >'.$str_jadi.'</option>';
    		      $result = $result.$opt;
		    }
		}else{
			foreach($data->result() as $row)
			{	
			  ...
			  $opt='<option value=\''.$row->$value.'\' '.$selected.'>'.$row->$mid.'</option>';
			  $result=$result.$opt;
			}
		}
		return '<select id=\''.$name.'\' name=\''.$name.'\' '.$tamb.' ><option value=\'0\'>'.$title.'</option>'
			.$result.
			'</select>';
	}	
	...
	function build_select_common_edit($name,$data,$value,$mid,$title,$key,$tamb='')
	{
		...
		if(is_array($mid))
		{
		   ...
		   {	
		     ...
		     $opt='<option value=\''.$row->$value.'\' '.$selected.' '.$check.' >'.$str_jadi.'</option>';
    		     $result = $result.$opt;
		    }
		}else{
			foreach($data->result() as $row)
